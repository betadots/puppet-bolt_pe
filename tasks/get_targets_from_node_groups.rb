...
def run(node_group)
  ...
  raise StandardError, "ERROR #{get_rule_response.code} - #{get_rule_response.message} - #{get_rule_response.code.class} - IDS: #{ids} - URI: #{get_rule_uri}" unless get_rule_response.code == '200'
